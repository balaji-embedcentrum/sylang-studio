...
function MessageItemComponent({
  ...
  // Stable scalar dep — `message` ref changes every render during streaming,
  // which would re-fire this effect and storm setState. `thinkingStatusLabel`
  // also doesn't need to restart the timer.
  const thinkingStartedAt = rawTimestamp(message) ?? null
  useEffect(() => {
    if (!thinking || hasText) {
  ...
  }, [hasText, thinking, thinkingStartedAt])
  ...
  // Stable scalar deps — avoid `message` ref churn during streaming, which
  // otherwise re-fires this effect every render and storms setState.
  const messageStatus = message.status
  const messageTimestamp = rawTimestamp(message)
  useEffect(() => {
    if (!isUser || messageStatus !== 'sending') {
      setIsStuckSending(false)
      return
    }
    const elapsed = messageTimestamp ? Date.now() - messageTimestamp : 0
    ...
    return () => window.clearTimeout(timer)
  }, [isUser, messageStatus, messageTimestamp])
